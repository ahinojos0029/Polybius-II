    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                if g.current_state == STATE_PLAYING:
                ...
                if event.key in (pygame.K_UP, pygame.K_w):
                    ...
                    g.pause_selection = (g.pause_selection + 1) % pause_options_count
                elif event.key in (pygame.K_LEFT, pygame.K_RIGHT, pygame.K_RETURN, pygame.K_SPACE, pygame.K_a, pygame.K_d):
                    if g.pause_selection == 0:
                        g.current_state = STATE_PLAYING
                    elif g.pause_selection == 1:
                        if event.key in (pygame.K_LEFT, pygame.K_a):
                            settings["master_volume"] = max(0.0, round(settings["master_volume"] - 0.1, 1))
                        else:
                            settings["master_volume"] = min(1.0, round(settings["master_volume"] + 0.1, 1))
                        ...
                        settings["colorblind_mode"] = not settings.get("colorblind_mode", False)
                        save_settings(settings)
            ...
            # events.py (Excerpt for STATE_OPTIONS)
            elif g.current_state == STATE_OPTIONS:
                categories_count = 3
                ...
                if g.options_category == 0:
                    if event.key in (pygame.K_UP, pygame.K_w):
                        g.active_category_idx = (g.active_category_idx - 1) % categories_count
                    elif event.key in (pygame.K_DOWN, pygame.K_s):
                        g.active_category_idx = (g.active_category_idx + 1) % categories_count
                ...
                # Mode 1: Adjusting items inside the chosen category
                    ...
                    # Determine item count dynamically based on the active category
                    if g.active_category_idx == 0:
                        ...
                        items_count = 2 if g.overdrive_unlocked else 1  # Gameplay & Extras
                    ...
                    if event.key in (pygame.K_UP, pygame.K_w):
                        ...
                        # Pop back out to category selection
                        g.options_category = 0
                    elif event.key in (pygame.K_RIGHT, pygame.K_RETURN, pygame.K_SPACE, pygame.K_d):
                        # CONTROLS CATEGORY
                        if g.active_category_idx == 0:
                        ...
                        # AUDIO & VIDEO CATEGORY
                            ...
                            if g.options_item_idx == 0:
                                # Handled by left/right or toggle, let's make enter/space step volume or toggle
                                pass
                            elif g.options_item_idx == 1:
                        ...
                        # GAMEPLAY & EXTRAS CATEGORY
                            ...
                            if g.options_item_idx == 0:
                                settings["discord_rpc_enabled"] = not settings["discord_rpc_enabled"]
                    ...
                    # Allow volume adjustment with Left/Right arrows when slider/item is selected
                ...
                if event.key == pygame.K_SPACE and g.shoot_cooldown <= 0:
                    ...
                    if g.shotgun_active:
                        for offset in [-0.15, 0.0, 0.15]:
                            g.bullets.append({"angle": g.player_angle + offset, "dist": g.player_distance})
                    else:
